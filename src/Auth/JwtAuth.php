...
final class JwtAuth
{
     ...
     */
    public function createJwt(User $user, array $requestedScopes = Role::ALL_VALUES, array $requestedState = IsActive::ALL_VALUES): JWT\Token\Plain
    {
        ...
        $awardedState = array_filter(
            array_unique(array_merge($requestedState, [IsActive::INACTIVE->value])),
            fn($isActive) => $user->hasIsActive($isActive),
        );
        ...
        $token = $this->config->builder()
            ...
            ->withClaim('state', array_values($awardedState))
